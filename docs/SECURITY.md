...
| P52 | Low | **The shell could reach a tablet mixed**: cache-first revalidates per file, so a session that ends early leaves some files updated and others not, and the worker's hand-written `VERSAO` never said the app had changed. Seen for real — a new `index.html` with the aba EMITIR next to an `app.js` whose router had no such route | **Fixed** — `GET /sw.js` injects a digest of `static/` as the version, and `install` fetches with `cache: "reload"`. Content changes, version changes, the whole shell is refetched and the previous cache is dropped |
