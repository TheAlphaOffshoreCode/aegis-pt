...
| P52 | Low | **The shell could reach a tablet mixed**: cache-first revalidates per file, so a session that ends early leaves some files updated and others not, and the worker's hand-written `VERSAO` never said the app had changed. Seen for real — a new `index.html` with the aba EMITIR next to an `app.js` whose router had no such route | **Fixed** — two causes, two fixes. `GET /sw.js` injects a digest of `static/` as the version and `install` fetches with `cache: "reload"`, so a changed shell is refetched whole and the previous cache dropped. And the shell now ships `Cache-Control: no-cache` (on the `304` as well): without it Starlette sends only `ETag`/`Last-Modified`, and the browser's heuristic freshness expires per file — which is the same mixture arriving by a route the service worker never touches |
